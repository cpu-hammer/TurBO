...
from lib import parse_cmd,run_playbook,get_default,save_tune_conf,find_exist_task_result,\
    divide_config,parse_result,_print,get_default_narrow
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  logger=logging.getLogger('run-bo-pp')
  handler=logging.FileHandler('run_information/run-bo-all-pp.txt')
  ...
  for task_id in tqdm(range(test_config.optimizer.iter_limit)):
    if task_id == 0:
      ...
      try:
          sampled_config_numeric, sampled_config = optimizer.get_conf(default_conf,task_id)
      except StopIteration:
        return
    if task_id == 49 :
        for one_sampled_config in sampled_config:
            confs = save_tune_conf(task_id, tune_conf, one_sampled_config)
            sampled_os_config, sampled_app_config = divide_config(one_sampled_config, os_setting=os_setting,
