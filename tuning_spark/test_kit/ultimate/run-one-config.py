...
from lib import parse_cmd,run_playbook,get_default,find_exist_task_result,\
    divide_config,parse_result,_print,save_tune_conf
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  logger = logging.getLogger('run-one-config')
  handler = logging.FileHandler('run_information/run-one-config.txt')
  handler.setFormatter(logging.Formatter('[%(asctime)s][%(name)s][%(levelname)s][%(message)s]'))
  logger.addHandler(handler)
  sampled_config_numeric, sampled_config = None, get_default(app_setting)
  temp_config = [3863,3962,1518,385,1035855877,41,3,110,82,51]
  for i ,j in zip(range(len(temp_config)),tune_configs):
      sampled_config[j] = temp_config[i]
  ...
  for rep in range(3):
      await single_test(
          task_name=test_config.task_name, task_id=task_id, rep=rep, tester=tester, testee=testee,
          slave1=slave1,
          slave2=slave2,
      ...
      result = parse_result(
          tester_name=test_config.tester, result_dir=result_dir, task_id=task_id, rep=0, printer=_print
      )
      result_list.append(- result)
      _print(f'{task_id} - {rep}: done.')
  metric_result = mean(result_list) if len(result_list) > 0 else .0
  logger.info('the result of task_id {} is {}'.format(task_id, abs(metric_result)))
