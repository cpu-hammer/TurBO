...
class RestuneOptimizer():# define of bo,and set the value of space /conf
  ...
  def get_conf(self):
    ...
    if self.bo.space.Y is None or len(self.bo.space.Y) < 5:
    ...
    if x_max in self.bo.space:
      x_max = self.bo.space.random_points(1)[0]
    return x_max,self._convert_to_dict(x_max)
  def _convert_to_dict(self, x_array):
    return dict(zip(self.space, x_array))

class RestuneBayesianOptimizer(RestuneOptimizer):
