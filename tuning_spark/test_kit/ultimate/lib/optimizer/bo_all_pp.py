...
class BOallppBayesianOptimizer(BOppOptimizer):
    ...
    def get_conf(self, default_conf,task_id):
        ...
        if length >= 3:
            ...
            print("pseudo-point is come from:{}".format(choose_number))
            pseudo_point_x = []
            pseudo_point_y = []
        ...
        if task_id == 49:
          new_conff = []
          for x_array in pseudo_point_x:
            new_x_array = self._convert_to_dict(x_array)
            for key in new_x_array:
              new_x_array[key] = self._rescale(new_x_array[key], bo_space[key])
            new_conf = default_conf
            for key in new_x_array:
              new_conf[key] = int(new_x_array[key])
            new_conff.append(new_conf.copy())
          return pseudo_point_x, new_conff
        for key in samples:
